...
function IterationComponent2() {
  ...
  //3. input 태그 state로 관리
  let [topic, setTopic] = useState('');



  
  //4. button 클릭시 input state 값을 배열에 추가
  let handleClick = () => {
    let obj = {id: list[list.length -1].id + 1 //id는 list의 마지막 값 + 1
      , topic: topic};

    let newList = list.concat(obj); //베열.concat(배열), 배열.concat(오브젝트)
    console.log(newList);
  ...
  let handleRemove = (id) => { //클릭한 대상의 id값
    ...
    let newList = list.filter((value) => {
      return value.id != id; //return에 실린 값이 true인 데이터만 필터링
    })
